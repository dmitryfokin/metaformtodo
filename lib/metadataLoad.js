...
const initMetadata = (metadata) => {
  const keys = ['app', 'config', 'kinds', 'modules', 'services'];
  for (const key of keys) {
    if (!(key in metadata)) metadata[key] = {};
  };
};

module.exports = async (sandbox, kinds) => {
  for (const kindKey of Object.keys(kinds)) {
    ...
    app[`${kind.kindPath}Path`] = path.join(app.appPath, kind.kindPath);
    const kindData = [];

    await readFiles(app[`${kind.kindPath}Path`], kindData, sandbox);
