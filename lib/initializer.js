...
module.exports = async (appSandbox) => {
  ...
  for (const i of inits) {
    ...
    if (i.module.beforeInit) await i.module.beforeInit(appSandbox, i.initializerConfig);
  };
  for (const i of inits) {
    console.log(`initialize ${i.initializerConfig.name}`);
    if (i.module.init) await i.module.init(appSandbox, i.initializerConfig);
  };
  for (const i of inits) {
    console.log(`after: initialize ${i.initializerConfig.name}`);
    if (i.module.afterInit) await i.module.afterInit(appSandbox, i.initializerConfig);
  };
};
